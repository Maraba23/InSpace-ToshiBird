...
def fase1_game(window, assets, state):
    if state['vidas'] == 3:
        window.blit(pygame.image.load(assets['fases_3vidas']), (0, 0))
    elif state['vidas'] == 2:
        window.blit(pygame.image.load(assets['fases_2vidas']), (0, 0))
    elif state['vidas'] == 1:
        window.blit(pygame.image.load(assets['fases_1vidas']), (0, 0))
    character = pygame.image.load(assets['character'])
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1 and not state['is_moving']:
                ...
                vel_x = 10 * math.cos(angle)
                vel_y = 10 * math.sin(angle)
